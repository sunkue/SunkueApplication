...
{
	ScrollEvents_.push(std::move(ScrollEvent{xoffset, yoffset, clk::now()}));
}

void MouseEventManager::MouseButton(GLFWwindow* window, int key, int action, int modifiers)
{
	ButtonEvents_.push(std::move(ButtonEvent{ key, action, modifiers, clk::now() }));
}

void MouseEventManager::CursorPosition(GLFWwindow* window, double xpos, double ypos)
{
	PosEvents_.push(std::move(PosEvent{ xpos, ypos, clk::now() }));
}
